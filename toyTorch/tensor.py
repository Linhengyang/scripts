...
# 2 size, 定义 tensor 的维度大小, 比如 (4, 3, 2) 代表在维度 (0, 1, 2) 的大小: 
#   维度0上有4个元素, 
...
## 对于 non-overlapping 的 tensor data, size 和 stride 之间存在一定的数字关系
# 1. strides[-1] 永远等于 1
# 2. strides[-2] 等于 size[-1], 因为 strides[-2] 代表了 在倒数第二维度偏移一个单位, 需要 meta-data 上的偏移量。
#    在 non-overlapping 的前提下, 这个量就代表了 在倒数第一维度最多允许的偏移量，即 倒数第一维度的长度.
# 3. strides[-3] 等于 size[-2] * size[-1], 因为 strides[-3] 代表了 在倒数第三维度偏移一个单位, 需要 meta-data 上的偏移量。
#    在 non-overlapping 的前提下, 这个量代表偏移 最多倒数第二维度长度次偏移, 而每一次倒数第二维度上的偏移，都代表了 倒数第一维度长度次偏移。
#    故 得到了 size[-2] * size[-1] 的结果。
# 4. 其他剩余维度的关系类似: 在 non-overlapping 的前提下, strides 和 size 之间存在累乘的关系


# 但是在通用的视角下, 基于 meta-data 之上的 size 和 stride 不存在数字关系: size 是 size, stride 是 stride,
# size 代表 各个 dimension, 在给定 前面低维 dimension 之后, 在当前维度 可以最多有多少次偏移量, 即为当前维度的 length-1
# 比如 tensor X 的 size 是 (d0, d1, d2, d3, d4), 代表在 dim0 可以最多有 d0-1 次偏移(d0个位置); 选定 dim0 后, 在 dim1 可以最多有 d1-1 次偏移(d1个位置)
...
# 给定 meta-data, 给定 size 和 stride(相同长度), size 和 stride 除了正整数外无其他要求, 即可从 meta-data 中寻址, 构建满足的 tensor
...
# 对于一个 size 为 (d_0, d_1, ..., d_i, ..., d_n), stride 为 (s_0, s_1, ..., s_i, ..., s_n). 现在要 unfold dimension i
...
# 所以 前面维度的 size d_0, ..., d_i-1 和 前面维度的stride s_0, ..., s_i-1 都没有变化。

# unfold 的关键在于, 原来 size 为 (d_i,...,d_n), stride 为 (s_i,...,s_n) 的 tensor, 要如何作 窗口宽度为 window_size, 滑动跨度为 step 的 unfold 操作?
...
# 原 tensor 在第 i 维上, stride 是 s_i, 长度是 d_i 个 size 为(d_i+1,...d_n)/stride为(s_i+1,...s_n) 的 原sub-tensor
...
# 长度是 num_windows 个 
#   "由 window_size 个 原sub-tensor, 即 size 为(window_size, d_i+1,...d_n)/stride为(s_i, s_i+1,...s_n) 的tensor permute 的 新sub-tensor"
